...
public class Camera {
    ...
    public Camera() {
        ...
        lastX = InputProcessor.getMouseX();
        lastY = InputProcessor.getMouseY();
    }

    public void update() {
        // --- Mouse look ---
        double mouseX = InputProcessor.getMouseX();
        double mouseY = InputProcessor.getMouseY();
        ...
        if (InputProcessor.isKeyPressed(GLFW_KEY_W)) velocity.add(new Vector3f(forward).mul(ACCEL));
        if (InputProcessor.isKeyPressed(GLFW_KEY_S)) velocity.sub(new Vector3f(forward).mul(ACCEL));
        if (InputProcessor.isKeyPressed(GLFW_KEY_A)) velocity.sub(new Vector3f(right).mul(ACCEL));
        if (InputProcessor.isKeyPressed(GLFW_KEY_D)) velocity.add(new Vector3f(right).mul(ACCEL));
        if (InputProcessor.isKeyPressed(GLFW_KEY_SPACE)) velocity.add(new Vector3f(up).mul(ACCEL));
        if (InputProcessor.isKeyPressed(GLFW_KEY_LEFT_SHIFT)) velocity.sub(new Vector3f(up).mul(ACCEL));
